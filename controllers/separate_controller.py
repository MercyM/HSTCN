...
class SeparateMAC(BasicMAC):
    ...
    def forward(self, ep_batch, t, test_mode=False, t_glob=0, train_mode=False):
        ...
        if self.args.name == "group":
            agent_outs, self.hidden_states, loss_cs, diss_loss, ce_loss = self.agent.forward(agent_inputs,
    ...
    def _build_agents(self, input_shape):
        if self.args.name == "group":
            self.agent = agent_REGISTRY[self.args.agent_macro](input_shape, self.args)
        else:
    ...
    def _build_inputs(self, batch, t):
        ...
        if self.args.obs_last_action:  # True for QMix
            if t == 0:
                if self.args.name == "group":
                    inputs.append(th.zeros_like(batch["macro_actions_onehot"][:, t]))  # last actions are empty
                else:
