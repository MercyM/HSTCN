...
class HAVENLearner:
    def __init__(self, mac, macro_mac, value_mac, scheme, logger, args):
        ...
        if args.value_mixer is not None:
            if args.value_mixer == "vdn":
                ...
                self.value_mixer = GraphMixer(args)
        ...
        if args.macro_mixer is not None:
            if args.macro_mixer == "vdn":
                ...
                self.macro_mixer = GraphMixer(args)
    ...
    def value_train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if self.args.name == "group":
            self.macro_mac.init_latent(batch.batch_size)
        for t in range(batch.max_seq_length):
            if self.args.name == "group":
                agent_outs, _, _, _ = self.macro_mac.forward(batch, t=t)
            else:
                agent_outs = self.macro_mac.forward(batch, t=t)
            mac_out.append(agent_outs)
            hidden_states.append(self.macro_mac.hidden_states.view(batch.batch_size, self.args.n_agents, -1))
        ...
        if self.args.macro_mixer == 'graphmix':
            max_qvals, local_rewards, alive_agents_mask = self.macro_mixer(max_qvals,
                                                                           batch["state"][:, :-1],
    ...
    def macro_train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        self.macro_mac.init_hidden(batch.batch_size)
        if self.args.name == "group":
            indicator, latent, latent_vae = self.macro_mac.init_latent(batch.batch_size)
            reg_loss = 0
            dis_loss = 0
            ce_loss = 0
        ...
        for t in range(batch.max_seq_length):
            if self.args.name == "group":
                agent_outs, loss_, dis_loss_, ce_loss_ = self.macro_mac.forward(batch, t=t, t_glob=t_env,
                                                                                train_mode=True)
                reg_loss += loss_
                dis_loss += dis_loss_
                ce_loss += ce_loss_
        ...
        if self.args.name == "group":
            self.target_macro_mac.init_latent(batch.batch_size)

        for t in range(batch.max_seq_length):
            if self.args.name == "group":
                target_agent_outs, loss_cs_target, _, _ = self.target_macro_mac.forward(batch, t=t)
            else:
                target_agent_outs = self.target_macro_mac.forward(batch, t=t)

            target_mac_out.append(target_agent_outs)
            target_hidden_states.append(
                self.target_macro_mac.hidden_states.view(batch.batch_size, self.args.n_agents, -1))
        ...
        if self.args.macro_mixer == 'graphmix':
            ...
            Q_total, local_rewards, alive_agents_mask = self.macro_mixer(chosen_action_qvals,
                                                                         batch["state"][:, :-1],
                                                                         agent_obs=batch["obs"][:, :-1],
                                                                         team_rewards=rewards,
                                                                         hidden_states=hidden_states[:, :-1]
            ...
            target_Q_total = self.target_macro_mixer(target_max_qvals,
                                                     batch["state"][:, 1:],
                                                     agent_obs=batch["obs"][:, 1:],
    ...
    def load_models(self, path):
        ...
        if self.value_mixer is not None:
            self.value_mixer.load_state_dict(
                th.load("{}/value_mixer.th".format(path), map_location=lambda storage, loc: storage))
        self.value_optimiser.load_state_dict(
            th.load("{}/value_opt.th".format(path), map_location=lambda storage, loc: storage))
        self.macro_mac.load_models(path)
        # Not quite right but I don't want to save target networks
        self.target_macro_mac.load_models(path)
        if self.macro_mixer is not None:
            self.macro_mixer.load_state_dict(
                th.load("{}/macro_mixer.th".format(path), map_location=lambda storage, loc: storage))
        self.macro_optimiser.load_state_dict(
            th.load("{}/macro_opt.th".format(path), map_location=lambda storage, loc: storage))
    ...
    def calc_intrinsic_reward(self, batch, macro_batch):
        ...
        if self.args.name == "group":
            self.macro_mac.init_latent(macro_batch.batch_size)
        for t in range(macro_batch.max_seq_length):
            if self.args.name == "group":
                agent_outs, _, _, _ = self.macro_mac.forward(macro_batch, t=t)
            else:
        ...
        if gap != 0:
            origin_reward = th.cat([origin_reward, th.zeros(
                [intrinsic_reward.size(0), intrinsic_reward.size(1) * self.args.k - origin_reward.size(1), 1]).cuda()],
                                   dim=1)
        ...
        intrinsic_reward = intrinsic_reward.view(intrinsic_reward.size(0), -1,
                                                 1 if self.args.mixer is not None else self.args.n_agents)
        intrinsic_reward = (intrinsic_reward[:, :batch.max_seq_length - 1] * mask).detach()
        intrinsic_reward = self.args.intrinsic_switch * intrinsic_reward + self.args.reward_switch * batch["reward"][:,
