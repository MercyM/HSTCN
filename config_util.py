...
def get_config(algorithm, minigame):
    ...
    with open(config_dir.format('config', "{}.yaml".format('default')), "r", encoding='utf-8') as f:
        try:
            default_config = yaml.load(f, Loader=yaml.FullLoader)
        except yaml.YAMLError as exc:
            assert False, "default.yaml error: {}".format(exc)

    with open(config_dir2.format('config', 'envs', "{}.yaml".format('sc2_beta')), "r", encoding='utf-8') as f:
        try:
    ...
    with open(config_dir2.format('config', 'algs', "{}.yaml".format(algorithm)), "r", encoding='utf-8') as f:
        try:
