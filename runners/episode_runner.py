...
class EpisodeRunner:
    ...
    def setup(self, scheme, macro_scheme, groups, preprocess, macro_preprocess, mac, macro_mac, value_mac,):
        self.new_batch = partial(EpisodeBatch, scheme, groups, self.batch_size, self.episode_limit + 1,
                                 preprocess=preprocess, device=self.args.device)
        self.new_macro_batch = partial(EpisodeBatch, macro_scheme, groups, self.batch_size,
                                       (self.episode_limit // self.args.k) + 1 + (
                                                   self.episode_limit % self.args.k != 0),
    ...
    def run(self, test_mode=False):
        ...
        if self.args.name == "group":
            self.macro_mac.init_latent(batch_size=self.batch_size)
